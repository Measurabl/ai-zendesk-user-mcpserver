...
- **The skill** (`skills/setup/SKILL.md`) only runs the scripts in
  ...
  and the copy into the plugin's own folder (`install-server.sh`) are
  pre-approved through `allowed-tools`; the Node download, the config edit, the
  restart, and uninstall each ask for approval.
- **Node.js.** `ensure-node.sh` reuses any Node 20+ already on the Mac
  (Homebrew, `/usr/local`, PATH, nvm), skipping version-manager shims that
  would not work outside a shell. Otherwise it downloads the pinned Node.js LTS
  from nodejs.org into `~/.local/share/zendesk-mcp/node/`, verified against
  nodejs.org's `SHASUMS256.txt` and against a checksum pinned in the script.
- **Install location.** `~/.local/share/zendesk-mcp/` holds `server/`, `node/`
  (if downloaded), and `VERSION`. The server never runs from inside the plugin
  directory, whose path changes on every plugin update.
...
- **Verification.** `verify.mjs` checks the config entry, the files and the
  Node binary, then starts the installed server and completes a real MCP
  `initialize` + `tools/list` handshake over stdio. Neither request touches
  Zendesk, so no sign-in is triggered.
- **Tokens.** The server keeps the OAuth token at
  ...
  old manual install; people who signed in before are not asked again.
...
| Path | Purpose |
| --- | --- |
| `.claude-plugin/plugin.json` | Plugin manifest. Its `version` is what triggers the organization sync. |
...
2. Run `pnpm build:plugin --bump`. It moves `version` in `plugin.json` and in
   the root `.claude-plugin/marketplace.json` to today's date (`YYYY.M.D`, or
   `YYYY.M.D+N` for a second release the same day), rebuilds `server/`, and
   prints the bundle's size and checksum.
3. Add an entry to `CHANGELOG.md`.
4. Open a PR. Two checks guard this directory and should be required on `main`:
   **Plugin version** (fails when `plugins/zendesk-mcp/**` changed without a
   version bump) and **Bundle fresh** (rebuilds the bundle from the PR's
   sources and fails when it differs and the PR touched `src/`, the build
   script, `tsconfig.json` or the plugin). A dependency-only PR that would
   change the bundle only warns; the next release rebuilds with current
   dependencies. The check also runs `claude plugin validate`.
...
Local checks: `pnpm test` covers the manifests and `merge-config.mjs`;
`claude plugin validate plugins/zendesk-mcp` checks the structure;
...
and `XDG_CONFIG_HOME` so they can be exercised against scratch paths, and
`ZENDESK_MCP_FORCE_DOWNLOAD=1` forces the Node download. Never test against a
real `claude_desktop_config.json`.
...
1. Merge the PR here; make **Plugin version** and **Bundle fresh** required
   checks on `main`.
...
approval prompts an AE sees; and the Okta sign-in end to end.
